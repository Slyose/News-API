...
exports.fetchArticles = (sort_by, order, topic) => {
  ...
  if (!topic) {
    queryStr += `GROUP BY articles.article_id ` + orderBy;
    ...
    queryStr += `WHERE topic = $1 GROUP BY articles.article_id ` + orderBy;
    return db.query(queryStr, [topic]).then(({ rows: articles }) => {
      if (articles.length === 0) {
        return Promise.reject({
          status: 404,
          msg: "Articles not found",
